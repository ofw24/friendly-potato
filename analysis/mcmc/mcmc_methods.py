...
from analysis.mcmc.not_mcmc_methods import ale, data_spline, residuals
...
def gibbs_sampler(data: pd.DataFrame, country: str, beta_ols: np.array, N: int, burn: int) -> np.array:
    ...
    # Splines
    # rs = residuals(data, country)
    # spl = data_spline(actual_life_exps, actual_life_exps-rs, order=1) # THIS CAN NEVER BE CHANGED
    # epsilon_ct = norm(loc=0, scale=omega*spl(actual_life_exps[-1])).rvs()
